...
describe("Тестирование макроса обезличивания:", function()
    ...
    describe("Тесты ФИО #FIO_all", function()
        ...
        it("Маскировка полного ФИО #FIO_1", function()
    ...
    describe("Тесты ИНН #inn_all", function ()
        ...
        it("Маскирует ИНН физического лица (12 цифр) #inn_1", function()
        ...
        it("Маскирует ИНН юридического лица (10 цифр) #inn_2", function()
            currentSelectionText = "ИНН организации: 1234567890."
            _G.RunSanitizer()
            assert.are.equal("ИНН организации: [ИНН].", lastMessageBoxText)
        end)

        it("НЕ маскирует слишком короткие номера (9 цифр) #inn_3", function()
    ...
    describe("Тесты СНИЛС #snils_all", function()
        ...
        it("Маскирует СНИЛС в стандартном формате #snils_1", function()
    ...
    describe("Тесты адресов #addr_all", function ()
        ...
        it("Маскирует стандартный город #addr_3", function()
            currentSelectionText = "Адрес: г. Москва, центр."
            _G.RunSanitizer()
            assert.are.equal("Адрес: [ГОРОД], центр.", lastMessageBoxText)
        end)

        it("Маскирует двойные названия городов #addr_4", function()
        ...
        it("Маскирует деревни и поселки #addr_6", function()
        ...
        it("Маскирует двойные названия улиц #addr_7", function()
            currentSelectionText = "проживаю на улице Малой Бронной, д. 10"
            _G.RunSanitizer()
            assert.are.equal("проживаю на [УЛИЦА]", lastMessageBoxText)
        end)

        it("Не маскирует остаток предложения после улицы #addr_8", function()
    ...
    describe("Тесты дат #date_all", function ()
        ...
        it("Маскирует полную дату в формате ДД.ММ.ГГГГ #date_1", function()
            currentSelectionText = "Документ от 15.03.2024 подписан."
            _G.RunSanitizer()
            assert.are.equal("Документ от [ДАТА] подписан.", lastMessageBoxText)
        end)

        it("Маскирует короткую дату в формате ДД.ММ.ГГ #date_2", function()
     ...
     describe("Тестирование паттерна [НОМЕР] num_all", function()
        ...
        it("Маскирует номер школы, оставляя само слово #num_1", function()
            currentSelectionText = "Ученик школы № 15 переведен."
            _G.RunSanitizer()
            assert.are.equal("Ученик школы [НОМЕР] переведен.", lastMessageBoxText)
        end)

        it("Маскирует сложные номера законов и договоров #num_2", function()
        ...
        it("Автоматически ловит неизвестные документы #num_3", function()
